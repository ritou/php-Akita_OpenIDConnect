<?php
/**
 * Akita_OpenIDConnect_Util_JOSE_JWT
 *
 * JSON Web Token
 *
 * @category  JOSE
...
class Akita_OpenIDConnect_Util_JOSE_JWT
{
    ...
    public function getTokenString($payload_is_array=false){
        ...
        if(!empty($this->_signature)){
            $token .= Akita_OpenIDConnect_Util_Base64::urlEncode($this->_signature);
        }
        return $token;
    }
    ...
    static public function getPayload($jwt, $return_is_array=false){
        ...
        if($return_is_array){
            $payload = json_decode(Akita_OpenIDConnect_Util_Base64::urlDecode($part[1]),true);
        }else{
            $payload = Akita_OpenIDConnect_Util_Base64::urlDecode($part[1]);
        }
        return $payload;
    }
